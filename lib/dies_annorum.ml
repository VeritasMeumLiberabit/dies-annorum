...
let first_sunday_of_advent year =
  ...
  let christmas_weekday_number =
    CalendarLib.Date.int_of_day christmas_weekday
  in
  let advent_length =
    CalendarLib.Date.Period.day (7 + 7 + 7 + christmas_weekday_number)
  in
...
let baptism_of_our_lord_transferred year =
  ...
  | _ ->
      CalendarLib.Date.add (epiphany_sunday year)
        (CalendarLib.Date.Period.day 7)
...
let christ_the_king year =
  CalendarLib.Date.rem
    (first_sunday_of_advent year)
    (CalendarLib.Date.Period.day 7)
...
let christmas_start office year =
  match office with
  | Office.Completorium | Office.Vesperas ->
      CalendarLib.Date.rem (christmas year) (CalendarLib.Date.Period.day 1)
  | _ -> christmas year
...
let easter_end office year =
  match office with
  | Office.Completorium ->
      CalendarLib.Date.rem (pentecost year) (CalendarLib.Date.Period.day 1)
  | _ -> pentecost year

(* Season Calculations *)

let advent_feast (module L : Language.Language) region date office =
  ...
  if date < advent_start || date >= christmas_start office year then
    ...
    let week =
      1
      + CalendarLib.Date.Period.nb_days (CalendarLib.Date.sub date advent_start)
        / 7
    in
    let rank, office_name =
      ...
      | Sat, Office.Vesperas | Sat, Office.Completorium ->
          (Rank.Primary, L.office_to_string ~kind:Office.First office)
      | Sun, Office.Vesperas | Sun, Office.Completorium ->
          (Rank.Primary, L.office_to_string ~kind:Office.Second office)
      | Sun, _ -> (Rank.Primary, L.office_to_string office)
      | _ -> (Rank.Primary, L.office_to_string office)
    in
    [{office = office_name; title = "placeholder"; rank; season; week; color}]

(* Endpoints *)

let get_celebrations (module L : Language.Language) region office date =
  let year = CalendarLib.Date.year date in
  (* The Advent Season.
   *
  ...
  if date >= advent_start office year && date < christmas_start office year then
    advent_feast (module L) region date office
    (*
    ...
    date >= christmas_start office year || date <= baptism_of_our_lord year
  then
    [ { office = "Vespers";
        title = "placeholder";
  ...
  else if date >= tridduum_start office year && date <= triddum_end office year
  then
    [ { office = "Vespers";
        title = "placeholder";
        rank = Rank.SolemnityGeneral;
